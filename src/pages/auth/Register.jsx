		...
		>
			<div>
				...
				<p className="mt-2 text-sm text-[var(--muted)]">
					Crea una cuenta conectada al backend.
				</p>
			</div>
			...
			<label className="grid gap-1.5 text-sm font-medium text-[var(--primary)]">
				...
				<div className="relative">
					...
					<input
						...
						onChange={onChange}
						placeholder="********"
						className="w-full rounded-xl border bg-white px-9 py-2.5 pr-10 text-[var(--fg)] outline-none focus:ring-2"
						style={{
							borderColor: "#0e544b",
							"--tw-ring-color": "#0e544b"
						}}
					...
					<button
						...
						className="absolute right-2 top-2.5 rounded-md p-1 text-[var(--secondary)] transition hover:bg-black/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--secondary)]"
					>
						{showPassword ? (
							<EyeOff size={18} />
						) : (
			...
			{error ? (
				<p className="m-0 rounded-xl border border-red-500/30 bg-red-50 px-3 py-2 text-sm text-red-700">
					{error}
				</p>
			) : null}

			<button
				className="inline-flex items-center justify-center rounded-xl px-4 py-2.5 text-sm font-semibold text-white transition disabled:cursor-not-allowed disabled:opacity-70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0e544b]"
				style={{ background: "#0e544b" }}
				type="submit"
				disabled={loading}
				onMouseOver={(e) =>
					(e.currentTarget.style.background = "#09332e")
				}
				onMouseOut={(e) =>
					(e.currentTarget.style.background = "#0e544b")
				}
			>
			...
			<p className="m-0 text-center text-sm text-[var(--muted)]">
				...
				<Link
					to="/login"
					className="rounded-xl px-3 py-1 font-semibold text-white transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0e544b]"
					style={{ background: "#0e544b" }}
					onMouseOver={(e) =>
						(e.currentTarget.style.background = "#09332e")
					}
